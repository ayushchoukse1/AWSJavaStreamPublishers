package com.amazonaws.achoukse.s3;
 ...
 * (/Users/achoukse/.aws/credentials) where the sample code will load the
 * credentials from.
 * <p>
...
public class S3Sample {
	...
	public static void main(String[] args) throws IOException {
		...
		/*
		 * The ProfileCredentialsProvider will return your [New Profile] credential
		...
		try {
			...
			throw new AmazonClientException("Cannot load the credentials from the credential profiles file. "
					...
					+ "location (/Users/achoukse/.aws/credentials), and is in valid format.", e);
		}

		AmazonS3 s3 = AmazonS3ClientBuilder.standard().withCredentials(new AWSStaticCredentialsProvider(credentials))
				.withRegion("us-east-1").build();

		String bucketName = "achoukse-anothertest";
		String key = null;
		...
		try {
			...
			for (int i = 0; i < 10; i++) {
				key = "ObjectFolder/MyObjectKey" + UUID.randomUUID();
				/*
	...
	private static File createSampleFile() throws IOException {
		File file = File.createTempFile("aws-achoukse", ".txt");
		file.deleteOnExit();
